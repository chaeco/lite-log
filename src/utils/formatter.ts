 ...
 * 将时间戳字符串按格式简化输出
 */
function formatTimestamp(ts: string, fmt: 'iso' | 'time' | 'datetime'): string {
  switch (fmt) {
...
export class LogFormatter {
  ...
  private buildPlainText(entry: LogEntry, includeTimestamp = true): string {
    const { format } = this.settings
    const parts: string[] = []
  ...
  private safeStringify(obj: any, indent?: number): string {
    ...
    try {
      ...
      try { return String(obj) } catch { return '[Unable to serialize]' }
    }
  }
}
