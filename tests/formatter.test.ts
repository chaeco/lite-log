...
describe('LogFormatter.formatMessage', () => {
  ...
  it('omits name when includeName is false', () => {
    const f = new LogFormatter(
      baseSettings({ format: { ...baseSettings().format, includeName: false } })
    );
  ...
  it('omits location when includeStack is false', () => {
    const f = new LogFormatter(
      baseSettings({ format: { ...baseSettings().format, includeStack: false } })
    );
...
describe('LogFormatter timestamp formats', () => {
  it('iso returns full ISO 8601 string', () => {
    const f = new LogFormatter(
      baseSettings({ format: { ...baseSettings().format, timestampFormat: 'iso' } })
    );
  ...
  it('datetime returns YYYY-MM-DD HH:mm:ss.mmm', () => {
    const f = new LogFormatter(
      baseSettings({ format: { ...baseSettings().format, timestampFormat: 'datetime' } })
    );
